...
/*
var AnimalObject= function(name,noise) {
  this.name= name;
  this.noise= noise;
  this.makeNoise= function() { return noise; };
}

var dog= new AnimalObject("dog","woof");
var cat= new AnimalObject("cat","meow");
var bird= new AnimalObject("bird","hu-hu");
var mouse= new AnimalObject("mouse","squeak");
//*/

///*
var dog= {
  name: "Pete", noise: "woof",
...
var bird= {
  name: "Tweety", noise: "hu-hu",
  makeNoise: function() { return this.noise; },
};

var mouse= {
  name: "Mickey", noise: "squeak",
  makeNoise: function() { return this.noise; },
};
//*/
...
for (var i=0;i<animals.length;i++) {
  animals[i].hunger= 10;
  console.log(`${animals[i].name}.hunger= ${animals[i].hunger}`);
}
...
for (var i=0;i<animals.length;i++) {
  animals[i].walk= function() { this.hunger--; return `${this.name} took a walk`; };
  console.log(`${animals[i].walk()}; ${animals[i].name}.hunger= ${animals[i].hunger}`);
}
...
///*
for (var i=0;i<animals.length;i++) {
  animals[i].makeFriend= function(name) { this.friend= name; };
  animals[i].makeFriend(animals[(i+2)%animals.length]);
  console.log(`${animals[i].name}'s friend is ${animals[i].friend.name} and ${animals[i].friend.makeNoise()}`);
}
//*/
...
for (var i=0;i<animals.length;i++) {
  console.log(`images/${this.name}.jpg`);
  animals[i].appear= function() {document.querySelector('img').setAttribute('src',`images/${this.name}.jpg`);};
}
